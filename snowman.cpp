...
std::string snowman(int dna) {
  ...
  if (!(dna > minimalBorderOfLegitDna && dna < maximalBorderOfLegitDna &&
        isBase3(dna))) {
...
std::string buildSnowman(int HNLRXYTB) {
  const int ten = 10; //* To take the digits out of HNLRXYTB we need to take a
                      //modulo of ten to retreive the last number and divide by
                      //ten to get access to the next last number.
  ...
  std::array<std::array<std::string, 4>, 2> H = {
      {{"     ", " ___ ", "  _  ", " ___ "},
       {"_===_", ".....", " /_\\ ", "(_*_)"}}};
  std::array<std::string, 4> N = {",", ".", "_", " "};
  std::array<std::string, 4> E = {
      ".", "o", "O", "-"}; //*E for Eye, instead of two equal matrises R & L.
